...
// functions called when mouse movement is there

$("td").mousedown(function () {
	var index = $("td").index(this);
	// var ci = $("td").parent().index(this)
...
function moveStartOrEnd(prevIndex, newIndex, startOrEnd) {
	...
	if (startOrEnd == "start") {
		...
		// console.log("Moving end to [" + newCellX + ", " + newCellY + "]")s
	}
	clearBoard(keepWalls = true);
	return;
}
...
function executeAlgo() {
	if (algorithm == "Depth-First Search (DFS)") {
		...
		visited[startCell[0]][startCell[1]] = 1;
		cellsToAnimate.push([[startCell[0], startCell[1]], "visited"]);
...
function DFS(i, j, visited) {
	...
	for (var k = 0; k < neighbors.length; k++) {
		...
		if (!visited[m][n]) {
			visited[m][n] = 1;
			cellsToAnimate.push([[m, n], "visited"]);
			var pathFound = DFS(m, n, visited);
			if (pathFound) {
				cellsToAnimate.push([[m, n], "success"]);
				return true;
			}
		}
	}
...
function BFS() {
	...
	cellsToAnimate.push(startCell, "visited");
	...
	while (!myQueue.empty()) {
		...
		for (var k = 0; k < neighbors.length; k++) {
			...
			visited[m][n] = true;
			prev[m][n] = [r, c];
			cellsToAnimate.push([neighbors[k], "visited"]);
